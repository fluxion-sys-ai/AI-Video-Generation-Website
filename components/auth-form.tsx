...
import { signIn, signInWithPassword, signUpWithPassword } from "@/lib/auth";
  ...
  return (
    <main className="mx-auto flex max-w-sm flex-col px-6 py-20">
      ...
      <form onSubmit={submit} className="space-y-4">
        <div>
          <label htmlFor="email" className="mb-1.5 block text-sm text-fg-soft-2">{identifierLabel}</label>
          <input
            id="email" type={identifierLabel === "Email" ? "email" : "text"} autoComplete="username" required value={email} onChange={(e) => setEmail(e.target.value)}
        ...
        <button type="submit" disabled={busy} className="w-full rounded-[10px] bg-accent px-4 py-2.5 font-medium text-ink transition-colors hover:bg-accent-hover disabled:cursor-not-allowed disabled:opacity-60">
          {busy ? "Please wait…" : title}
        </button>
      </form>
      <p className="mt-6 text-sm text-fg-soft-2">
        {mode === "login" ? (
          <>No account? <Link href="/signup" className="text-blue hover:underline">Sign up</Link></>
        ) : (
          <>Have an account? <Link href="/login" className="text-blue hover:underline">Log in</Link></>
        )}
      </p>
    </main>
  );
